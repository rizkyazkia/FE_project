...
const TableRecommendation = () => {
  ...
  const familyMemberId = selectedRec?.student?.familyMember?.id;
  ...
  const searchData = (e) => {
    ...
    setPage(0);
    setKeyword(query);
    recommendationMutate("recommendations", { revalidate: true });
  };
  ...
  } else if (filteredRecomend?.length > 0) {
    tableContent = filteredRecomend.map((rec, idx) => (
      <tr key={rec.id}>
            ...
            <button
              ...
              onClick={() =>
                setSelectedRec({
                  ...rec,
                  nomorUrut:
                    recommendationData.page * recommendationData.limit +
                    idx +
                    1,
                })
              }
                ...
                <div id="surat" className="p-6 h-full">
                  {selectedRec && (
                    <Surat
                      ...
                      nomorUrut={selectedRec.nomorUrut}
